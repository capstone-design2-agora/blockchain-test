#!/usr/bin/env bash
# Run phased IBFT benchmarks at multiple TPS targets and organize reports.
...
for tps in "${TPS_VALUES[@]}"; do
  ...
  for run_id in $(seq 1 "${RUNS_PER_TPS}"); do
    ...
    dest_path="${tps_dir}/${date_prefix}_ibft_phase_${tps}_${run_id}.md"
